class Form {
  ...
  authorise(data) {
    const errorMsg =(document.getElementsByClassName('authorise-error-message'));

    return new Promise((resolve, reject) => {
      axios.post('login', data).then((response) => {
        if (response.data.status !== 'Success' && errorMsg.length ===0) {
          const errorMessage = document.createElement('p');
          errorMessage.innerText = 'Invalid login or password';
          errorMessage.classList.add('text-danger','p-3','font-italic','authorise-error-message');
          this._form.append(errorMessage);
        } else{
